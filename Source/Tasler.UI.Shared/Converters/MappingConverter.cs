...
namespace Tasler.Windows.Data
{
	...
	///         <taz:MappingConverter x:Key="severityToImageMappingConverter">
	...
	///     <UserControl.Resources>
	///         <taz:MappingConverter x:Key="severityToImageMappingConverter">
	///             <taz:Mapping Key="{x:Static app:Severity.Info}" Value="images/Info.png" />
	///             <taz:Mapping Key="{x:Static app:Severity.Warning}" Value="images/Warning.png" />
	///             <taz:Mapping Key="{x:Static app:Severity.Error}" Value="images/Error.png" />
	///         </taz:MappingConverter>
	///     </UserControl.Resources>
	/// </code>
	/// </example>
	...
	public class MappingConverter : IValueConverter
	{
		...
		{
			get { return this._mappings ?? (_mappings = new MappingCollection()); }
		}
		...
		{
			object mappedValue = this._mappings.Find(value);
//			Debug.WriteLine(string.Format("MappingConverter.Convert: {0}={1}\n", value, mappedValue));
	...
	public class Mapping<TKey, TValue> : IMapping
	{
		...
		object IMapping.Key { get { return this.Key; } }
		object IMapping.Value { get { return this.Value; } }
	...
	public class MappingCollection : IList<IMapping>, IList
	{
		...
		{
			this._dictionary.Remove(this._list[index].Key);
