using System.Runtime.InteropServices;
using System.Runtime.InteropServices.Marshalling;
using Tasler.Interop.Com;

namespace Tasler.Interop.Shell;

[GeneratedComInterface(StringMarshalling = StringMarshalling.Utf16)]
[Guid("0E700BE1-9DB6-11d1-A1CE-00C04FD75D13")]
public partial interface IEnumExtraSearch
{
	/// <param name="elementCount"></param>
	/// <param name="elements"></param>
	/// <summary>
	/// Retrieves up to <paramref name="elementCount"/> EXTRASEARCH entries from the enumerator into the provided buffer.
	/// </summary>
	/// <param name="elementCount">Maximum number of elements to retrieve.</param>
	/// <param name="elements">Array that receives the retrieved EXTRASEARCH entries; the array must have length at least <paramref name="elementCount"/>.</param>
	/// <param name="elementsFetched">The number of elements actually written into <paramref name="elements"/>.</param>
	/// <returns>An HRESULT value indicating success or failure (`S_OK` on success); the value encodes the result of the COM operation.</returns>
	[PreserveSig]
	int Next(int elementCount, [MarshalUsing(CountElementName = nameof(elementCount))] [Out] EXTRASEARCH[] elements, out int elementsFetched);

	/// <summary>
	/// Advances the enumeration by the specified number of elements.
	/// </summary>
	/// <param name="elementsToSkip">Number of elements to advance the enumerator.</param>
	/// <returns>An HRESULT value indicating success or failure as an integer.</returns>
	[PreserveSig]
	int Skip(int elementsToSkip);

	/// <summary>
/// Resets the enumeration to the initial position so the next call to Next retrieves elements from the start.
/// </summary>
void Reset();

	/// <summary>
/// Creates a new enumerator that duplicates the current enumerator's state.
/// </summary>
/// <returns>An <see cref="IEnumExtraSearch"/> instance positioned at the same point in the enumeration as the current one.</returns>
IEnumExtraSearch Clone();
}

[StructLayout(LayoutKind.Sequential)]
public struct EXTRASEARCH
{
	public Guid GuidSearch;
	private unsafe fixed char _friendlyName[80];
	private unsafe fixed char _url[2084];

	public readonly string FriendlyName
	{
		get
		{
			unsafe
			{
				fixed (char* friendlyName = _friendlyName)
				{
					return new(friendlyName);
				}
			}
		}
	}

	public readonly string Url
	{
		get
		{
			unsafe
			{
				fixed (char* url = _url)
				{
					return new(url);
				}
			}
		}
	}
}