...
namespace Tasler.ComponentModel
{
	...
	public static class PropertyObserver
	{
		...
		{
			return Subscribe(source, PropertySupport.ExtractPropertyName(propertyExpression), s => handlerAction((TSource)s));
		}
		...
		public static IPropertyObserverItem Subscribe<TSource, TProperty>(
			...
			where TSource : class, INotifyPropertyChanged
		{
			return Subscribe(source, PropertySupport.ExtractPropertyName(propertyExpression), eventHandler);
		}
		#endregion Methods

		#region Private Implementation
		private static IPropertyObserverItem Subscribe(INotifyPropertyChanged source, string propertyName, Action<INotifyPropertyChanged> handlerAction)
		{
			var observerItem = new ObserverItem(source, propertyName, handlerAction);
			return observerItem;
		}

		private static IPropertyObserverItem Subscribe(INotifyPropertyChanged source, string propertyName, PropertyChangedEventHandler eventHandler)
		{
